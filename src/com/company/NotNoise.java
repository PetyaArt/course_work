...
public class NotNoise extends JFrame{
    ...
    public static void main(String[] args) throws IOException {
        File mFileImage = new File("src//com//company//image//NotNoise.png");
        ...
        for (int i = 0; i < raster.getHeight(); i++) { ////первая производная
            for (int j = 0; j < raster.getWidth() - 1; j++) {
                    ...
                    raster.setSample(j, i, 0, buffer3 * 150);
                    raster.setSample(j, i, 1, buffer3 * 150);
                    raster.setSample(j, i, 2, buffer3 * 150);
        ...
        for (int i = 0; i < rasterSave.getHeight(); i++) { //вторая производная
            for (int j = 1; j < rasterSave.getWidth() - 1; j++) {
                ...
                rasterSave.setSample(j, i, 0, buffer3 * 150);
                rasterSave.setSample(j, i, 1, buffer3 * 150);
                rasterSave.setSample(j, i, 2, buffer3 * 150);
                if (rasterSave.getWidth() - 2  <= j){
        ...
        mas2[0] = 490 - 0;
        mas2[rasterSave.getWidth() - 1] = 490 - 0;
        for (int i = 0; i < 1; i++) {
            for (int j = 1; j < rasterSave.getWidth() - 1; j++) {
                mas2[j] = 490 - ((rasterLoad.getSample(j + 1, i, 0) + rasterLoad.getSample(j - 1, i, 0) - (rasterLoad.getSample(j, i, 0) * 2)) * 150);
            }
        }
        y2 = mas2;
        n2 = y2.length;
        ...
        new com.company.NotNoise().setVisible(true);
    }
